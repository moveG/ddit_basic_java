...
public class StringClass {
	...
	public static void main(String[] args) {
		 ...
		 * - charAt(): 특정 인덱스에 위치한 문자를 반환한다./ String(문자열)이 아닌 char(문자)를 반환한다.
		 * - substring(): 문자열의 특정 부분을 잘라서 반환한다.
		 * - indexOf(): 문자열 내의 특정 문자의 인덱스를 반환한다.
		 * - contains(): 문자열이 특정 문자열을 포함하고 있는지 여부를 반환한다./ 포함하고 있으면 true, 포함하고 있지 않으면 false를 반환한다.
		...
		String str1 = "문자열";
		String str2 = null;
		System.out.println(str1.equals(str2)); //문자열을 null과 비교할 수 있다.
//		System.out.println(str2.equals(str1)); //null은 문자열과 비교할 수 없다, null값을 가진 String타입 변수는 equals() 메서드를 소환할 수 없다.
		
		String str = "12345";
		int length = str.length();
		//length()는 for문에 자주 이용된다.
		//length()는 길이를 int타입으로 반환된다.
		for(int i = 0; i < length; i++){
			char charAt = str.charAt(i);
			System.out.println(charAt);
			//for문이 돌면서 charAt()을 통해 1, 2, 3, 4, 5를 반환한다.
		}
		...
		//substring()은 String타입을 반환한다.
		String sub2 = str.substring(5, 8);
		//5번 인덱스 ~ 8번 이전 인덱스(7번)까지 잘라서 반환한다.
		System.out.println(sub2);
		
		str = "수박 오렌지 귤 블루베리";
		int index = str.indexOf("오렌지");
		//indexOf()는 인덱스 값을 int타입으로 반환한다.
		System.out.println(index);
		//substring과 indexOf는 함께 사용하는 경우가 많다.
...
//			int price = Integer.parseInt(menu[i].substring(menu[i].indexOf(" ") + 1, menu[i].indexOf("원")));
		...
		//contains()은 문자열이 특정 문자열을 포함하고 있는지 여부를 반환한다./ 포함하고 있으면 true, 포함하고 있지 않으면 false를 반환한다.
		for(int i = 0; i < menu.length; i++){
