...
public class ExceptionHandling {
	...
	public static void main(String[] args) {
		 ...
		 * - 예외: 프로그램 코드에 의해 수습될 수 있는 다소 미약한 오류(처리가능)
		 * 
		 * 예외
		...
		try{
			...
			e.printStackTrace(); //발생한 에러의 메시지를 출력해줌
		}catch(NullPointerException e){
			//아무것도 들어있지 않아도 에러 처리 가능, 타입 여러개 가능
		}catch(Exception e){
			e.printStackTrace();
		}
...
//		System.out.println(10 / 0);
	...
	//에러메시지가 여러개 발생하면 가장 위에서부터 찾되, 내가 만든 오류를 찾아 확인한다.
	//위에서부터 내가 만든 것을 찾고 선택해서 오류를 확인한다.
