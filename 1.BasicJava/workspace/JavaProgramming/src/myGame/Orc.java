...
public class Orc {
	...
	Orc(String name, int hp, int att, int def, int oGolds){
		this.name = name;
		this.maxHp = hp;
		this.hp = this.maxHp;
		this.att = att;
...
//히어로 공격
	void attack1(Hero h){
		if(h.hp > 0) {
			...
			System.out.println(name + "가 공격으로 " + h.heroName + "에게 " + damage + "만큼 데미지를 주었습니다.");
...
//마법사 공격
	void attack2(Magician m){
		if(m.hp > 0){
			...
			System.out.println(name + "가 공격으로 " + m.magicianName + "에게 " + damage + "만큼 데미지를 주었습니다.");
